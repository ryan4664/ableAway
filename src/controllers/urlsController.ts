...
export const urlsController = async (c: any) => {
  ...
  const domainCards = Object.entries(domainGroups).map(([domain, hotels]) => {
    ...
    const latestDate = new Date(Math.max(...hotels.map((h: any) => new Date(h.scrapedAt).getTime()))).toLocaleDateString();
          ...
          <div class="bg-gradient-to-r from-blue-500 to-green-500 p-1">
            <div class="bg-white p-6 rounded-lg">
              <div class="flex items-start justify-between mb-4">
                ...
                <div class="ml-4">
                  <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gradient-to-r ${badgeColor}">
...
export const hotelDetailController = async (c: any) => {
  ...
  const confidenceBar = hotel.extractionConfidence ? `
    <div>
